...
public abstract class ChecksumFileSystem extends FilterFileSystem {
  ...
  public void setConf(Configuration conf) {
    super.setConf(conf);
    if (conf != null) {
      bytesPerChecksum = conf.getInt("io.bytes.per.checksum", P2PConstants.IO_BYTES_PER_CHECKSUM);
    }
  }
  ...
  private int getSumBufferSize(int bytesPerSum, int bufferSize) {
    int defaultBufferSize = getConf().getInt("io.file.buffer.size", P2PConstants.IO_FILE_BUFFER_SIZE);
  ...
  private static class ChecksumFSInputChecker extends FSInputChecker {
    ...
    public ChecksumFSInputChecker(ChecksumFileSystem fs, Path file)
      throws IOException {
      this(fs, file, fs.getConf().getInt("io.file.buffer.size", P2PConstants.IO_FILE_BUFFER_SIZE));
    }
