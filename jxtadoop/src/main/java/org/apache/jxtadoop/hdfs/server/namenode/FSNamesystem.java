...
import org.apache.jxtadoop.hdfs.DFSUtil;
...
import org.apache.jxtadoop.hdfs.server.common.GenerationStamp;
...
import org.apache.jxtadoop.hdfs.server.namenode.BlocksMap.BlockInfo;
import org.apache.jxtadoop.hdfs.server.namenode.metrics.FSNamesystemMBean;
import org.apache.jxtadoop.hdfs.server.namenode.metrics.FSNamesystemMetrics;
...
import org.apache.jxtadoop.hdfs.server.protocol.DisallowedDatanodeException;
...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  private static final void logAuditEvent(UserGroupInformation ugi,
      PeerID pid, String cmd, String src, String dst,
      FileStatus stat) {
  ...
  private void setConfigurationParameters(Configuration conf) 
    ...
    this.replicator = new ReplicationTargetChooser(
                         ...
                         this,
                         clusterMap);
    this.defaultReplication = conf.getInt("dfs.replication", P2PConstants.DEFAULT_DFS_REPLICATION);
    ...
    this.heartbeatExpireInterval = 2 * heartbeatRecheckInterval + 4 * heartbeatInterval;
    this.replicationRecheckInterval = 
      conf.getInt("dfs.replication.interval", P2PConstants.DEFAULT_DFS_REPLICATION) * 1000L;
    this.defaultBlockSize = conf.getLong("dfs.block.size", P2PConstants.DEFAULT_BLOCK_SIZE);
  ...
  synchronized BlocksWithLocations getBlocks(DatanodeID datanode, long size)
    ...
    if (node == null) {
      NameNode.stateChangeLog.warn("BLOCK* NameSystem.getBlocks: "
          + "Asking for blocks from an unrecorded node " + datanode.getPeerId());
      throw new IllegalArgumentException(
          "Unexpected exception.  Got getBlocks message for datanode " + 
          datanode.getPeerId() + ", but there is no info for it");
    }
  ...
  public LocatedBlock getAdditionalBlock(String src, 
	  ...
	  return getAdditionalBlock(src,clientName,"");
  }
  public LocatedBlock getAdditionalBlock(String src, 
                                         String clientName, String localDatanode
                                         ) throws IOException {
   ...
   * 
   * @see org.apache.jxtadoop.hdfs.server.datanode.DataNode#register()
   */
  public synchronized void registerDatanode(DatanodeRegistration nodeReg
                                            ) throws IOException {
    String dnAddress = Server.getRemotePeerID().toString();
    if (dnAddress == null) {
      // Mostly called inside an RPC.
      // But if not, use address passed by the data-node.
      dnAddress = nodeReg.getPeerId();
    }     
    ...
    DatanodeID dnReg = new DatanodeID(nodeReg.getPeerId(),
    ...
    NameNode.stateChangeLog.info(
                                 ...
                                 + "node registration from " + nodeReg.getPeerId()
                                 + " storage " + nodeReg.getStorageID());

    DatanodeDescriptor nodeS = datanodeMap.get(nodeReg.getStorageID());
    DatanodeDescriptor nodeN = host2DataNodeMap.getDatanodeByName(nodeReg.getPeerId());
      
    if (nodeN != null && nodeN != nodeS) {
      NameNode.LOG.info("BLOCK* NameSystem.registerDatanode: "
                        + "node from name: " + nodeN.getPeerId());
    ...
    if (nodeS != null) {
       ...
       */        
        NameNode.stateChangeLog.info( "BLOCK* NameSystem.registerDatanode: "
                                      + "node " + nodeS.getName()
                                      + " is replaced by " + nodeReg.getPeerId() + 
      ...
      nodeS.setHostName(nodeReg.getPeerId());
  ...
  synchronized public void removeDatanode(DatanodeID nodeID) 
    ...
    if (nodeInfo != null) {
      ...
      NameNode.stateChangeLog.warn("BLOCK* NameSystem.removeDatanode: "
                                   + nodeID.getPeerId() + " does not exist");
    }
  }
  ...
  public synchronized void blockReceived(DatanodeID nodeID,  
    ...
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug("BLOCK* NameSystem.blockReceived: "
                                    +block+" is received from " + nodeID.getPeerId());
    }
   ...
   */
  @Deprecated
  public JxtaSocketAddress getDFSNameNodeAddress() {
    return nameNodeJxtaAddress;
  }
  ...
  private boolean inHostsList(DatanodeID node, String ipAddr) {
    ...
    return (hostsList.isEmpty() || 
            ...
            hostsList.contains(node.getPeerId()) || 
  ...
  private boolean inExcludedHostsList(DatanodeID node, String ipAddr) {
    ...
    return  ((ipAddr != null && excludeList.contains(ipAddr)) ||
            excludeList.contains(node.getPeerId()) ||
            ((node instanceof DatanodeInfo) && 
  ...
  public DatanodeDescriptor getDatanode(DatanodeID nodeID) throws IOException {
    ...
    if (node == null) 
      return null;
    if (!node.getName().equals(nodeID.getPeerId())) {
