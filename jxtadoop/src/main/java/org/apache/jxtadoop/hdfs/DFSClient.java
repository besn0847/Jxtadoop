...
import org.apache.jxtadoop.fs.BlockLocation;
import org.apache.jxtadoop.fs.ChecksumException;
import org.apache.jxtadoop.fs.ContentSummary;
import org.apache.jxtadoop.fs.FSDataInputStream;
import org.apache.jxtadoop.fs.FSInputChecker;
import org.apache.jxtadoop.fs.FSInputStream;
import org.apache.jxtadoop.fs.FSOutputSummer;
import org.apache.jxtadoop.fs.FileStatus;
import org.apache.jxtadoop.fs.FileSystem;
import org.apache.jxtadoop.fs.MD5MD5CRC32FileChecksum;
import org.apache.jxtadoop.fs.Path;
import org.apache.jxtadoop.fs.Syncable;
...
import org.apache.jxtadoop.hdfs.protocol.DSQuotaExceededException;
...
import org.apache.jxtadoop.hdfs.protocol.NSQuotaExceededException;
...
import org.apache.jxtadoop.hdfs.server.namenode.NotReplicatedYetException;
...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  private DFSClientPeer dfspeer;
  private static DFSClient dfsclientObject;
  private String seed = "";
  private String localDatanodePID = "";
 
  public static DFSClient getDfsClient() {
	    return dfsclientObject;
	  } 
  
  public DFSClientPeer getDfsClientPeer() {
	  return dfspeer;
  }
  
  public static ClientProtocol createNamenode(Configuration conf) throws IOException {
	  return createNamenode(DFSClient.getDfsClient().getDfsClientPeer().getRpcSocketAddress(), conf);
  }

  public static ClientProtocol createNamenode( JxtaSocketAddress nameNodeAddr,
  ...
  private static ClientProtocol createRPCNamenode(JxtaSocketAddress nameNodeAddr,
      Configuration conf, UnixUserGroupInformation ugi) 
    throws IOException {
	  return (ClientProtocol)RPC.getProxy(ClientProtocol.class,
			  ClientProtocol.versionID, DFSClient.getDfsClient().getDfsClientPeer().getPeerGroup(),
			  nameNodeAddr, ugi, conf, NetUtils.getSocketFactory(conf, ClientProtocol.class));
  }
  ...
  static ClientDatanodeProtocol createClientDatanodeProtocolProxy (
	...
	JxtaSocketAddress jsa = DFSClient.getDfsClient().getDfsClientPeer().getRpcSocketAddress(datanodeid.getPeerId());
	PeerGroup pg = DFSClient.getDfsClient().getDfsClientPeer().getPeerGroup();
	  
    if (ClientDatanodeProtocol.LOG.isDebugEnabled()) {
      ClientDatanodeProtocol.LOG.info("ClientDatanodeProtocol addr=" + jsa);
    }
    
    return (ClientDatanodeProtocol)RPC.getProxy(ClientDatanodeProtocol.class, ClientDatanodeProtocol.versionID, pg, jsa, conf);
  }
        

  /**
   * Same as this(conf);
   * @see #DFSClient(Configuration)
   */
  public DFSClient(Configuration conf) throws IOException {
	  this(conf, (FileSystem.Statistics) null);
  }
  
  public DFSClient(Configuration conf,String seed) throws IOException {
	  this(conf);
	  this.seed = seed;
  }
  
  /**
   * Same as this(null, conf, stats);
   * @see #DFSClient(ClientProtocol, Configuration, org.apache.jxtadoop.fs.FileSystem.Statistics) 
   */
  public DFSClient(Configuration conf,
                   FileSystem.Statistics stats)
    throws IOException {
    this(null, conf, stats);
  }
  ...
  DFSClient(ClientProtocol rpcNamenode,
    ...
    this.dfspeer = new DFSClientPeer("DFS - "+ seed + System.getProperty("jxtadoop.datanode.id"));
    
    if (System.getProperties().containsKey("jxtadoop.local.datanode")) {
    	if(System.getProperty("jxtadoop.local.datanode").length() == P2PConstants.PEER_ID_LENGTH ) {
	    	localDatanodePID = System.getProperty("jxtadoop.local.datanode");
	    	LOG.debug("Local datanode id has been set thru command line : "+localDatanodePID);
	    }
    }
    
    if (localDatanodePID.isEmpty() && !System.getProperties().containsKey("jxtadoop.local.datanode")) {
    	if(dfspeer.hasLocalDatanode()) {
    		localDatanodePID = dfspeer.getLocalDatanodePeerID();
    		LOG.debug("Local datanode id has been gathered from local P2P dir structure : "+localDatanodePID);
    	}
    }
    
    if (localDatanodePID.length() ==0)
    	LOG.debug("There is no local datanode associated with this DFS client peer");
    
    dfspeer.initialize();
    dfspeer.start();
    
    LOG.debug("Info Socket timeout set to "+Integer.parseInt(conf.get("hadoop.p2p.info.timeout")));
    
    dfsclientObject = this;
    ...
    defaultBlockSize = conf.getLong("dfs.block.size", P2PConstants.DEFAULT_BLOCK_SIZE);
    defaultReplication = (short) conf.getInt("dfs.replication", P2PConstants.DEFAULT_DFS_REPLICATION);
    ...
    if(retry ==0) throw new RuntimeException("Cannot connect DFS client to namenode");
  ...
  public void init() {
	  if (NameNode.getNameNode() == null)
		  LOG.debug("This is not a NameNode DFS Client");
  }
  ...
  public String getLocalDatanode() {
	  return localDatanodePID;
  }
  ...
  public DFSInputStream open(String src) throws IOException {
    return open(src, conf.getInt("io.file.buffer.size", P2PConstants.IO_FILE_BUFFER_SIZE), true, null);
  }
  ...
  public OutputStream create(String src, 
    ...
    LOG.debug("Block size is : "+blockSize);
    LOG.debug(src + ": masked=" + masked);
    OutputStream result = new DFSOutputStream(src, masked,
        overwrite, replication, blockSize, progress, buffersize,
        conf.getInt("io.bytes.per.checksum", P2PConstants.IO_BYTES_PER_CHECKSUM));
  ...
  OutputStream append(String src, int buffersize, Progressable progress
    ...
    OutputStream result = new DFSOutputStream(src, buffersize, progress,
        lastBlock, stat, conf.getInt("io.bytes.per.checksum", P2PConstants.IO_BYTES_PER_CHECKSUM));
    leasechecker.put(src, result);
    return result;
  }
  ...
  public static MD5MD5CRC32FileChecksum getFileChecksum(String src,
    ...
    for(int i = 0; i < locatedblocks.size(); i++) {
      ...
      for(int j = 0; !done && j < datanodes.length; j++) {
    	...
    	JxtaSocket jsock = DFSClient.getDfsClient().getDfsClientPeer().getInfoSocket(datanodes[j].getName());
    	...
    	DataOutputStream out = new DataOutputStream(
                new BufferedOutputStream(jsock.getOutputStream()));
   ...
   * @see org.apache.jxtadoop.hdfs.protocol.ClientProtocol#setQuota(String, long, long)
   */
  ...
  class LeaseChecker implements Runnable {
     ...
     */
    public void run() {
    	// Changed from 0 to current time to avoid client lock
      long lastRenewed = System.currentTimeMillis();
  ...
  private static class DNAddrPair {
    ...
    DNAddrPair(DatanodeInfo info, JxtaSocketAddress addr) {
      this.info = info;
      this.addr = addr;
    }
  }
  ...
  public static class BlockReader extends FSInputChecker {
    ...
    private JxtaSocket dnSock; //for now just sending checksumOk.
    ...
    public static BlockReader newBlockReader(JxtaSocket sock, String file, long blockId, 
    ...
    public static BlockReader newBlockReader( JxtaSocket sock, String file,
                                       long blockId, 
                                       long genStamp,
                                       long startOffset, long len,
                                       int bufferSize, boolean verifyChecksum,
     ...
     */ 
    private void checksumOk(JxtaSocket sock) {
      try {
  ...
  class DFSInputStream extends FSInputStream {
    ...
    private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
      ...
      while (s == null) {
        ...
        JxtaSocketAddress targetAddr = retval.addr;

        try {
          // s = socketFactory.createSocket();
          // NetUtils.connect(s, targetAddr, socketTimeout);
          // NetUtils.connect(s, targetAddr, socketTimeout);
          s = DFSClient.getDfsClient().getDfsClientPeer().getInfoSocket(chosenNode.getPeerId());
    ...
    private DNAddrPair chooseDataNode(LocatedBlock block)
      ...
      while (true) {
        ...
        try {
          ...
          JxtaSocketAddress targetAddr = DFSClient.getDfsClient().getDfsClientPeer().getInfoSocketAddress(chosenNode.getPeerId());
    ...
    private void fetchBlockByteRange(LocatedBlock block, long start,
      ...
      while (dn == null && numAttempts-- > 0 ) {
        ...
        try {
          ...
          dn = DFSClient.getDfsClient().getDfsClientPeer().getInfoSocket(chosenNode.getPeerId());
          // dn.setSoTimeout(socketTimeout);
          dn.setSoTimeout(Integer.parseInt(conf.get("hadoop.p2p.info.timeout")));
  ...
  ****************************************************************/
  class DFSOutputStream extends FSOutputSummer implements Syncable {
    private JxtaSocket s;
    boolean closed = false;
  
    private String src;
    private ReliableOutputStream blockReliableStream;
    ...
    private class DataStreamer extends Daemon {
      ...
      public void run() {
        while (!closed && clientRunning) {
          ...
          synchronized (dataQueue) {
            ...
            try {
              ...
              if (blockStream == null) {
                ...
                LOG.debug("Starting new data streamer");
                this.setName("DataStreamer for file " + src +
                             " block " + block);
                response = new ResponseProcessor(nodes);
                LOG.debug("Kicking off the nodes responder");
                response.start();
              }
              ...
              LOG.debug("Creating byte buffer for streaming");
             ...
             /*LOG.debug("socket send buffer size : "+s.getSendBufferSize());
              LOG.debug("socket timeout : "+s.getSoTimeout());
              LOG.debug("socket receive buffer size : "+s.getReceiveBufferSize());
              LOG.debug("socket so linger : "+s.getSoLinger());
              LOG.debug("socket window size : "+s.getWindowSize());
              LOG.debug("buffer capacity : "+buf.capacity());
              LOG.debug("buffer array offset : "+buf.arrayOffset());
              LOG.debug("buffer limit : "+buf.limit());
              LOG.debug("buffer position : "+buf.position());
              LOG.debug("buffer remaining : "+buf.remaining());*/
              
              /*if (s.isClosed())
            	  LOG.debug("The streaming socket has been closed");
              else
            	  LOG.debug("The streaming socket is still open");
              
              if (blockReliableStream.isClosed())
            	  LOG.debug("The streaming reliable stream has been closed");
              else
            	  LOG.debug("The streaming reliable stream is still open");*/
            	  
              try {
            	  blockStream.write(buf.array(), buf.position(), buf.remaining());
              } catch (SocketTimeoutException ste) {
            	  LOG.warn("Failed to stream data to datanode (timeout) : "+ste.getMessage());
              } catch (IOException ioe) {
            	  LOG.warn("Failed to stream data to datanode : "+ioe.getMessage());
              } catch (Exception e) {
            	  LOG.warn("Failed to stream data to datanode : "+e.getMessage());
              }
              
              if (one.lastPacketInBlock) {
            	  // LOG.debug("Breakpoint 6.1");
            	  try {
            		  blockStream.writeInt(0); // indicate end-of-block 
            	  } catch (SocketTimeoutException ste) {
                	  LOG.warn("Failed to indicate end of block to datanode (timeout) : "+ste.getMessage());
                  }
              }
              ...
              try {
            	  ...
            	  LOG.warn("Exception while flushing block stream : "+e.getMessage());
              }
          ...
          // Added by FBE to cope with P2P set up
          synchronized (ackQueue) {
              while (!hasError && ackQueue.size() > 3 && clientRunning) {
                try {
      ...
      public void run() {
        ...
        while (!closed && clientRunning && !lastPacketInBlock) {
          ...
          try {
        	...
        	long seqno = -1;
            try {
            	seqno = blockReplyStream.readLong();
            } catch (SocketTimeoutException ste ) {
            	LOG.warn("Failed to verify seqno from datanode (timeout) : "+ste.getMessage());
            }
            ...
            for (int i = 0; i < targets.length && clientRunning; i++) {
            	short reply = -1;
            	try {
            	   reply = blockReplyStream.readShort();
              } catch (SocketTimeoutException ste ) {
              	LOG.warn("Failed to verify seqno from datanode (timeout) : "+ste.getMessage());
              }
              if (reply != DataTransferProtocol.OP_STATUS_SUCCESS) {
    ...
    DFSOutputStream(String src, FsPermission masked, boolean overwrite,
      ...
      // Create the DFS entry for the new file
      try {
        namenode.create(
      ...
      // Stream the blocks fromlocal file to remote DFS
      streamer.start();
    }
    ...
    private DatanodeInfo[] nextBlockOutputStream(String client) throws IOException {
      ...
      do {
        ...
        LOG.debug("Locating the block");		// This is where the blocks are assigned
        if(dfsclientObject.hasLocalDatanode())
        	lb = locateFollowingBlock(startTime,dfsclientObject.getLocalDatanode()); 	
        else
        ...
        //
        LOG.debug("Connecting to the first datanode in the list : "+nodes[0].getName());
    ...
    private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client,
      ...
      try {
        ...
        // s = socketFactory.createSocket();
        // s= DFSClient.getDfsClient().getDfsClientPeer().getInfoSocket(nodes[0].getPeerId());
        //int timeoutValue = 3000 * nodes.length + socketTimeout;
        // NetUtils.connect(s, target, timeoutValue);
        // s.setSoTimeout(timeoutValue);
        // s.setSoTimeout(Integer.parseInt(conf.get("hadoop.p2p.rpc.timeout")));
        ...
        s.setSoTimeout(Integer.parseInt(conf.get("hadoop.p2p.info.timeout")));
       	...
       	if(!s.isConnected() || s.isClosed()) {
       		LOG.fatal("Cannot reconnect socket");
       	}
        	   	
       	blockReliableStream = (ReliableOutputStream) s.getOutputStream(); 
       	
       	if(blockReliableStream.isClosed())
       		LOG.debug("Reliable output stream has been closed");
       	out = new DataOutputStream(new BufferedOutputStream(blockReliableStream));
        ...
        LOG.debug("Flushing in createBlockOutputStream" );
        out.flush();
	
        // receive ack for connect
        ReliableInputStream ris = (ReliableInputStream) s.getInputStream();
        if (ris.isInputShutdown())
        	LOG.debug("Reliable input stream has been shutdown");
        
        blockReplyStream = new DataInputStream(ris);
        
        try {
        	LOG.debug("Reading in createBlockOutputStream" );
        	firstBadLink = Text.readString(blockReplyStream);
        } catch (EOFException eof) {
        	LOG.warn("Failed to read on createBlockOutputStream" );
        	eof.printStackTrace();
        } catch (IOException ioe) {
        	LOG.warn("Failed to read on createBlockOutputStream" );
        	ioe.printStackTrace();
        ...
        LOG.debug("firstBadLink : "+firstBadLink);
    ...
    private LocatedBlock locateFollowingBlock(long start
    	...
    	return locateFollowingBlock(start,"");
    }
    
    /**
     * Selecting the datanodes where to place the blocks
     * 
     * @param start	
     * @param localdn the local datanode colocated with the dfs client
     * @return the located blocks with the associated datanodes
     * @throws IOException
     */
    private LocatedBlock locateFollowingBlock(long start, String localdn
                                              ) throws IOException {
    	// TODO: Modify the logic to force the first datanode to be the local datanode
      ...
      while (true) {
        ...
        while (true) {
          try {
            if (localdn.length() == 0) {
            	LOG.debug("Asking namenode to add the block "+src+" by client : "+clientName);
            	return namenode.addBlock(src, clientName);
            } else {
            	LOG.debug("Asking namenode to add the block "+src+" by client : "+clientName + " with first DN to be "+localdn);
            	return namenode.addBlock(src, clientName,localdn);
            }
        ...
        synchronized (dataQueue) {
          if (blockStream != null) {
            try {
            	...
            	LOG.warn("Failed to indicate end-of-block to datanode (timeout) : "+ ste);
            }
            blockStream.close();
            blockReplyStream.close();
          }
