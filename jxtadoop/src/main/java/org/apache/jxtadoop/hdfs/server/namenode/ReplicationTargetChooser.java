...
class ReplicationTargetChooser {
  ...
  DatanodeDescriptor[] chooseTarget(int numOfReplicas,
                                    DatanodeDescriptor writer,
                                    List<Node> excludedNodes,
                                    long blocksize, String localDatanode) {
    if (excludedNodes == null) {
      excludedNodes = new ArrayList<Node>();
    }
      
    ArrayList<DatanodeDescriptor> choosenDns = new ArrayList<DatanodeDescriptor>();
    
    if (localDatanode.length() > 0) {
    	/** TODO : **/
    	LOG.debug("Looking for existing local datanode in the DN descriptor list");
    	Host2NodesMap map = fs.getHost2DataNodeMap();
    	DatanodeDescriptor localdn = map.getDatanodeByHost(localDatanode);
    	
    	if(localdn != null) {
    		LOG.debug("Local datanode has been found in the map and is added to the choosen list");
    		choosenDns.add(localdn);
    		return choosenDns.toArray(new DatanodeDescriptor[choosenDns.size()]);
    	} 
    } else {
		LOG.debug("There is no local datanode; skipping");
	}
    
    DatanodeDescriptor[] targets = chooseTarget(numOfReplicas, writer, choosenDns, excludedNodes, blocksize);
    
    for (DatanodeDescriptor target : targets)
    	LOG.debug("Identified target : "+target.getHostName());
