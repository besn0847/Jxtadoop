...
class BlockReceiver implements java.io.Closeable, FSConstants {
  ...
  BlockReceiver(Block block, DataInputStream in, String inAddr,
    ...
    try{
      ...
      LOG.debug("Error in block receiver : "+ ioe.getMessage());
  ...
  public void close() throws IOException {
	LOG.debug("Closing blockreceiver");
  ...
  void flush() throws IOException {
	  LOG.debug("Flushing blockreceiver");
    if (checksumOut != null) {
      checksumOut.flush();
    }
    if (out != null) {
  ...
  private void verifyChunks( byte[] dataBuf, int dataOff, int len, 
                             ...
                             throws IOException {
	  LOG.debug("Verifying chunks in blockreceiver");
	  while (len > 0) {
  ...
  private void shiftBufData() {
	  LOG.debug("Shifting buffer data  blockreceiver");
  ...
  private int readNextPacket() throws IOException {
	  LOG.debug("Reading next packet  blockreceiver");
  ...
  private int receivePacket() throws IOException {
    ...
    LOG.debug("Enqueing pending ack : "+seqno);
    if (responder != null) {
    	LOG.debug("Responder is not null; Enqueing : {seqno = "+seqno+"; lastPacketInBlock : "+ lastPacketInBlock+"}");
  ...
  void writeChecksumHeader(DataOutputStream mirrorOut) throws IOException {
	  LOG.debug("Writing checksum header");
  ...
  void receiveBlock(
	  ...
	  LOG.debug("Receiving block in  blockreceiver");
  ...
  private void cleanupBlock() throws IOException {
	  LOG.debug("Cleaning up block in  blockreceiver");
